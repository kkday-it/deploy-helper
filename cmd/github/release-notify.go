		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			if inputSlackChannel != "" && inputSlackWebhook != "" {
				msg := &slack.WebhookMessage{
					...
					Text:    fmt.Sprintf("%s deploy success. version: %s <https://github.com/kkday-it/%s/releases/tag/%s|Click here> for details!\n%s", inputRepo, inputTag, inputRepo, inputTag, repoRelease.GetBody()),
				}
